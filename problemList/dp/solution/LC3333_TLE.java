...
public class LC3333_TLE {
    public int possibleStringCount(String word, int k) {
        ...
        // dp[0][0] = 1;
        dp1[0] = 1;
        int[] preSum = new int[sum + 2];
        Arrays.fill(preSum, 1);
        preSum[0] = 0;
        for(int i = 1;i <= n;i++){
            int[] dp2 = new int[sum + 1];
            int[] preSum2 = new int[sum + 2];
            for(int j = 0;j <= sum;j++){
                // for(int p = 0;p <= list.get(i - 1);p++){
                //     if(j - p >= 0) dp2[j] = (dp2[j] + dp1[j - p]) % MOD;
                // }
                dp2[j] = (preSum[j + 1] - preSum[Math.max(0, j - list.get(i - 1))] + MOD) % MOD;
                preSum2[j + 1] = (preSum2[j] + dp2[j]) % MOD;
            }
            dp1 = dp2;
            preSum = preSum2;
        }
    ...
    public static void main(String[] args) {
        ...
        int k = 7;
        System.out.println(new LC3333_TLE().possibleStringCount(word, k));
    }
}
