...
public class LC3165 {
    public int maximumSumSubsequence(int[] nums, int[][] queries) {
        ...
        SegmentTreeBasic segTree = new SegmentTreeBasic(n);
        segTree.build(1, 1, n, newNums);
        // for(int[] query : queries){
        //     segTree.modify(1, 1, n, query[0] + 1, query[1]);
        //     System.out.println(segTree.getRet());
        // }
        System.out.println("build complete...");
        segTree.modify(1, 1, n, 2, -2);
        return segTree.getRet();
    }
}
...
class SegmentTreeBasic {
    ...
    public void build(int o, int l, int r, int[] nums){
        if(l == r){
            ...
            dp[o][1] = nums[l];
            System.out.println("l = " + l + " r = " + r + " dp = " + Arrays.toString(dp[o]));
            return;
        }
        int mid = (l + r) >> 1;
    ...
    private void up(int o){
        dp[o][0] = Math.max(dp[o * 2][0] + Math.max(dp[o * 2 + 1][0], dp[o * 2 + 1][2]), dp[o * 2][1] + dp[o * 2 + 1][2]);
        dp[o][1] = Math.max(dp[o * 2][0] + Math.max(dp[o * 2 + 1][1], dp[o * 2 + 1][3]), dp[o * 2][1] + dp[o * 2 + 1][3]);
        dp[o][2] = Math.max(dp[o * 2][2] + Math.max(dp[o * 2 + 1][0], dp[o * 2 + 1][2]), dp[o * 2][3] + dp[o * 2 + 1][2]);
        dp[o][3] = Math.max(dp[o * 2][2] + Math.max(dp[o * 2 + 1][1], dp[o * 2 + 1][3]), dp[o * 2][3] + dp[o * 2 + 1][3]);
    }
    ...
    public void modify(int o, int l, int r, int idx, int val) {
        ...
        else modify(o * 1 + 1, mid + 1, r, idx, val);
        // 最后在回溯的过程中更新当前节点
        up(o);
        System.out.println("l = " + l + " r = " + r + " dp = " + Arrays.toString(dp[o]));
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new LC3165().maximumSumSubsequence(nums, queries));
    }
}
